...
import { getTextColorForBackground } from "@/lib/TextClr";
...
export default function MovieDetails({ id }) {
  ...
  const [hasMovie, setHasMovie] = useState(
    JSON.parse(localStorage.getItem("playlist").includes(id)) || false
  );
  ...
  const handleAddPlayList = (id) => {
    ...
    if (!existingPlaylist.includes(id)) {
      ...
      toast({
        title: "Couldn't Add",
        description: "This movie is already in your playlist!",
        variant: "error",
        action: (
          <Link href="/watchlist" passHref>
            <ToastAction altText="View">View</ToastAction>
  ...
  return (
    <>
        ...
        <div className="relative z-10 flex flex-col gap-3 col-span-2">
          ...
          <div className="my-2 flex items-center gap-2">
            ...
            <div
              ...
              className="w-10 h-10 rounded-full bg-zinc-50 text-zinc-900 flex items-center justify-center cursor-pointer"
            >
              {hasMovie ? <MdBookmarkAdded /> : <RiPlayListAddFill />}
            </div>
            <div
              onClick={handleShare}
              className="w-10 h-10 rounded-full bg-zinc-50 text-zinc-900 flex items-center justify-center cursor-pointer"
      ...
      <div className="p-5">
        {credits.length > 0 && <MovieCategory title={"Cast"} />}
        <CastSlider movies={credits} />
      </div>

      <div className="p-5">
        {relatedMovies.length > 0 && <MovieCategory title={"Related Movies"} />}
